...
private func renderAudioInput(inRefCon: UnsafeMutableRawPointer?, actionFlags: AudioUnitRenderActionFlags?, inTimeStamp: AudioTimeStamp?, inBusNumber: UInt32, inNumberOfFrames: UInt32, ioData: AudioBufferList?) -> OSStatus {
    print(inRefCon ?? "FUCK THIS STUPID BULLSHIT")
...
private func performFFT(data: UnsafeMutablePointer<Float>?, numberOfFrames: UInt32, soundBuffer: SoundBufferPtr, inBusNumber: UInt32) {
    ...
    let bufferLog2 = Int(round(log2(Double(numberOfFrames)))) //Int(round(log2(numberOfFrames)))
    var fftNormFactor: Float = 1.0 / (2 * Float(numberOfFrames))
...
class CoreAudioManager: NSObject, AudioManager {
    ...
    private var mSoundBuffer = [SoundBuffer]() //TODO - make dynamically sized based on files added
    ...
    func loadAudioFiles() {
        ...
        for i in 0..<sourcePaths.count {
            ...
            //Open the audio file
            var extAFref = ExtAudioFileRef(bitPattern: 0) //0 as? ExtAudioFileRef
...
//            //Release the reference to the file url
    ...
    func initializeAUGraph() {
        ...
        var result = OSStatus(0) //0 as? OSStatus
...
//    }
    ...
    func guitarFrequencyDataOfLength(_ size: UnsafeMutablePointer<UInt32>?) -> Float32? {
//        size = frequencyDataLength
//        return mSoundBuffer[0].frequencyData
        return 0
    }

    func drumsFrequencyDataOfLength(_ size: UnsafeMutablePointer<UInt32>?) -> Float32? {
//        size = frequencyDataLength
