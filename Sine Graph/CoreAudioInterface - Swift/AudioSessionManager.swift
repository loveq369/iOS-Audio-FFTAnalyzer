...
class AudioSessionManager: NSObject {
    ...
    @objc func handleInterruption(_ notification: Notification?) {
        ...
        print("AVAudioSession interrupted: \(interruptionType == UInt8(AVAudioSessionInterruptionTypeBegan) ? "Begin Interruption" : "End Interruption")")

        if interruptionType == UInt8(AVAudioSessionInterruptionTypeBegan) {
        ...
        } else if interruptionType == UInt8(AVAudioSessionInterruptionTypeEnded) {
            var error: Error? = nil
